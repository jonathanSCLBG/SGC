...
REV.cargarUsuarioSesion = async function () {
  ...
  if (String(data.tipo_usuario || "").toLowerCase() !== "revisor") {
...
REV.configurarBotonValidar = async function () {
  ...
  if (isValidada) {
    ...
    txtBtn.textContent = "Desvalidar Respuesta";
    return;
  }
...
REV.validarRespuesta = async function () {
  ...
  const preg = String(REV.state.notaContext.pregunta);
  const yaValidada = Number(REV.state.validadaPorPregunta[preg] || 0) === 1;

  if (yaValidada) {
    if (!confirm("¿Deseas desvalidar esta respuesta?")) return;

    sp.style.display = "inline-block";
    txt.textContent = "Desvalidando...";
    btn.disabled = true;
    lbl.textContent = "";
    ...
    try {
      const resp = await fetch("../php/desvalidar_respuesta.php", {
        method: "POST",
        body: fd
      });
      ...
      if (!resp.ok || !data.ok) {
        throw new Error(data.detalle || data.msg || "No se pudo desvalidar.");
      }

      alert(data.msg || "Respuesta desvalidada ✅");

      REV.state.validadaPorPregunta[preg] = 0;
      ...
      console.error(e);
      alert(e.message || "Error al desvalidar.");
    } finally {
  ...
  try {
    ...
    REV.state.validadaPorPregunta[preg] = 1;
